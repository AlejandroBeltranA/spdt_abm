...
class EnergyModel(mesa.Model):
    ...
    def _apply_intraday_profiles(self) -> None:
        ...
        for house in self.household_agents:
            ...
            if bucket == "electric":
                heat_mult = heat_base_elec
                dhw_share = dhw_base_elec
            else:
                heat_mult = heat_base_gas
                dhw_share = dhw_base_gas
    ...
    def _apply_climate_tick(self) -> None:
        ...
        if self.heating_months is not None:
            cur_month = (self._start_ts_utc + pd.to_timedelta(self.current_hour - 1, unit="h")).month
